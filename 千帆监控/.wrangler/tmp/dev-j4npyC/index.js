...
var WECOM_API = "https://qyapi.weixin.qq.com/cgi-bin/webhook/send";
...
__name(send, "send");
async function sendMarkdown(key, content) {
  return send(key, { msgtype: "markdown", markdown: { content } });
}
__name(sendMarkdown, "sendMarkdown");

// src/services/dedup.ts
async function isDuplicate(kv, type) {
...
async function updateHeartbeat(kv, agisoRunning) {
  await kv.put("heartbeat:last", JSON.stringify({
    time: (/* @__PURE__ */ new Date()).toISOString(),
    agiso_running: agisoRunning
  }));
}
__name(updateHeartbeat, "updateHeartbeat");
async function getLastHeartbeat(kv) {
  const data = await kv.get("heartbeat:last");
  if (!data) return null;
  return JSON.parse(data);
}
__name(getLastHeartbeat, "getLastHeartbeat");
...
__name(formatNewMessage, "formatNewMessage");
function formatProcessDown() {
  return [
    "## \u{1F534} \u53D1\u8D27\u52A9\u624B\u5DF2\u79BB\u7EBF",
    `> **\u65F6\u95F4**: ${now()}`,
    "> **\u72B6\u6001**: \u963F\u5947\u7D22\u53D1\u8D27\u52A9\u624B\u8FDB\u7A0B\u672A\u68C0\u6D4B\u5230",
    "",
    "\u8BF7\u68C0\u67E5 Windows \u7535\u8111\uFF0C\u91CD\u65B0\u542F\u52A8\u53D1\u8D27\u52A9\u624B\uFF01"
  ].join("\n");
}
__name(formatProcessDown, "formatProcessDown");
function formatHeartbeatTimeout(lastHeartbeat) {
  return [
    "## \u26A0\uFE0F \u76D1\u63A7\u811A\u672C\u79BB\u7EBF",
    `> **\u6700\u540E\u5FC3\u8DF3**: ${lastHeartbeat}`,
    `> **\u5F53\u524D\u65F6\u95F4**: ${now()}`,
    "",
    "\u8BF7\u68C0\u67E5 Windows \u7535\u8111\u662F\u5426\u6B63\u5E38\u8FD0\u884C"
  ].join("\n");
}
__name(formatHeartbeatTimeout, "formatHeartbeatTimeout");
...
function formatReport(type, message) {
  return [
    `## \u{1F4E2} \u76D1\u63A7\u4E0A\u62A5: ${type}`,
    `> **\u65F6\u95F4**: ${now()}`,
    message ? `> **\u8BE6\u60C5**: ${message}` : ""
...
function formatHeartbeatRecovered() {
  return [
    "## \u2705 \u76D1\u63A7\u811A\u672C\u5DF2\u6062\u590D",
    `> **\u65F6\u95F4**: ${now()}`,
    "> \u5FC3\u8DF3\u6B63\u5E38\uFF0C\u7CFB\u7EDF\u8FD0\u884C\u4E2D"
...
__name(formatHeartbeatRecovered, "formatHeartbeatRecovered");
function now() {
  return (/* @__PURE__ */ new Date()).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" });
}
__name(now, "now");

// src/routes/monitor-report.ts
var app = new Hono2();
app.use("/*", async (c, next) => {
...
app.post("/report", async (c) => {
  ...
  if (await isDuplicate(c.env.KV, type)) {
    return c.json({ ok: true, action: "deduped" });
  }
  ...
  switch (type) {
    ...
    case "flash_detected":
      ...
      break;
    case "process_down":
      content = formatProcessDown();
      break;
    case "error":
      content = formatError(message || "\u672A\u77E5\u9519\u8BEF");
      break;
    default:
      content = formatReport(type, message);
  }
...
app.post("/heartbeat", async (c) => {
  const body = await c.req.json();
  await updateHeartbeat(c.env.KV, body.agiso_running);
  if (!body.agiso_running) {
    const dup = await isDuplicate(c.env.KV, "process_down");
    if (!dup) {
      await sendMarkdown(c.env.WECOM_WEBHOOK_KEY, formatProcessDown());
    }
  }
  return c.json({ ok: true });
});
var monitor_report_default = app;

// src/routes/agiso-webhook.ts
var app2 = new Hono2();
app2.post("/webhook/agiso", async (c) => {
  const body = await c.req.json();
  ...
  const now2 = (/* @__PURE__ */ new Date()).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" });
  const content = [
    `## \u{1F6D2} \u963F\u5947\u7D22\u63A8\u9001: ${type}`,
    `> **\u65F6\u95F4**: ${now2}`,
...
var agiso_webhook_default = app2;

// src/index.ts
var app3 = new Hono2();
app3.use("/*", cors());
app3.get("/health", (c) => c.json({ ok: true, service: "xhs-notify" }));
app3.route("/", monitor_report_default);
app3.route("/", agiso_webhook_default);
app3.notFound((c) => c.json({ error: "Not Found" }, 404));
app3.onError((err, c) => {
...
var src_default = {
  fetch: app3.fetch,
  /**
   * Cron Trigger: 每 5 分钟检查 Windows 监控脚本心跳
   * 超过 5 分钟无心跳 → 发企微告警
   */
  async scheduled(_event, env, _ctx) {
    const heartbeat = await getLastHeartbeat(env.KV);
    if (!heartbeat) {
      return;
    }
    const lastTime = new Date(heartbeat.time).getTime();
    const elapsed = Date.now() - lastTime;
    const TEN_MINUTES = 10 * 60 * 1e3;
    if (elapsed > TEN_MINUTES) {
      const alreadyAlerted = await env.KV.get("heartbeat:alerted");
      if (!alreadyAlerted) {
        const localTime = new Date(heartbeat.time).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai" });
        await sendMarkdown(env.WECOM_WEBHOOK_KEY, formatHeartbeatTimeout(localTime));
        await env.KV.put("heartbeat:alerted", "1", { expirationTtl: 3600 });
      }
    } else {
      const wasAlerted = await env.KV.get("heartbeat:alerted");
      if (wasAlerted) {
        await env.KV.delete("heartbeat:alerted");
        await sendMarkdown(env.WECOM_WEBHOOK_KEY, formatHeartbeatRecovered());
      }
    }
  }
};
...
// .wrangler/tmp/bundle-v2tPFZ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-v2tPFZ/middleware-loader.entry.ts
